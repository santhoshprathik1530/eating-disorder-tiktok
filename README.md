...
Before those calls run, the script also queries OpenRouter's current models catalog and keeps only model IDs that are actually available at that moment. If none of your preferred models are live, it falls back to the cheapest currently available compatible paid models first, then free models.
